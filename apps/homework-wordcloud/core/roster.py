...
# -*- coding: utf-8 -*-
"""
roster.py — 原始名單 → 固定學生編號 → 對照表（HomeworkWordCloud 2.2）
====================================================================
**為什麼要有這個模組**

2.0 的學生編號是「依每個檔案內首次出現順序」給的，所以同一位同學在
Q01 可能是 `115-1_EC_3`、在 Q02 卻變成 `115-1_EC_7` —— 沒辦法跨題、跨週追蹤，
這是已經確認的缺陷。2.1 起改成：**先拿 TA 手上的原始名單產生一份固定對照表**，
之後每一次處理都照這份對照表給編號，整學期不變。

**流程**

    原始名單（Excel／CSV／Word／東華選課名單 PDF）
        │  prepare_codebook()
        ▼
    [{序號, 學號, 姓名, 班級}, …]
...
**與「姓名遮罩與學生編號」（NameMasker 2.2）完全相容**：欄位名、檔名、工作表名、
101 起的名單外登記規則都一樣，兩支程式可以共用同一份對照表。
本檔與 `NameMasker\\roster.py` 完全同步（只有檔頭這段說明不同）。
